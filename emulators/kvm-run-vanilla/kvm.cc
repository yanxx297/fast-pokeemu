...
void VCPU::Disasm(void *addr, FILE *f) {
  ...
  ud_init(&ud_obj);
  ud_set_mode(&ud_obj, 32 /* CPU_BITS */);
...
void VCPU::SingleStep(bool b) {
  struct kvm_guest_debug kgd;
  if (b) {
    kgd.control = KVM_GUESTDBG_ENABLE | KVM_GUESTDBG_SINGLESTEP;
  } else {
    kgd.control = 0;
  }
  ioctl(cpu_fd, KVM_SET_GUEST_DEBUG, &kgd);
}
