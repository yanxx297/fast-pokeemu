...
struct
  ...
  class frag_machine = object(self)
    ...
    val mutable custom_eip_hooks = []
		val mutable custom_store_hooks = []
		val mutable custom_load_hooks = []		

    method set_eip_hook f = 
      custom_eip_hooks <- f :: custom_eip_hooks;
	...
	extra_eip_hooks;
			List.iter (fun fn -> (fn eip)) custom_eip_hooks;	(*From emuFuzzBALL*)
      self#watchpoint
    ...
    method run_jump_hooks last_insn last_eip eip =
      self#jump_hook last_insn last_eip eip
		...
		method run_store_hooks (addr:int64) (ty:int) (exp:string) = (
			let run_hook fn = (
				fn addr ty exp
				) in
			List.iter run_hook custom_store_hooks)
			
		method set_load_hook f = 
			custom_load_hooks <- f :: custom_load_hooks			

		method run_load_hooks (addr:int64) (ty:int) = (
			let run_hook fn = (
				fn addr ty
				) in
			List.iter run_hook custom_load_hooks
			)
...
(* 	self#store_byte_conc 0x60000037L 0xff; (* base high *) *)
    ...
    method store_symbolic_byte_expr addr expr =
      self#store_byte addr (D.from_symbolic expr)
