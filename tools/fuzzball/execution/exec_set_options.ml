...
let opt_symbolic_bytes_expr = ref []
let opt_symbolic_bytes_expr_str = ref []
let opt_symbolic_bytes_expr_files = ref []
...
let opt_extra_condition_files = ref []
...
let symbolic_state_cmdline_opts =
  [
    ...
    ("-symbolic-byte-expr", Arg.String
       (add_delimited_num_str_pair opt_symbolic_bytes_expr_str '#'),
     "addr#expr Make a memory byte symbolic (assigning it exp)");
    ("-symbolic-byte-expr-from-file", Arg.String
       (add_delimited_num_str_pair opt_symbolic_bytes_expr_files '='),
     "addr=file Make a memory byte symbolic (assigning it exp from file)");
...
let explore_cmdline_opts =
  [
    ...
    ("-ignore-path", Arg.String
      (fun s -> opt_ignored_paths :=
       (Int64.of_string s) :: !opt_ignored_paths),
     "addr Ignore paths going through addr");		
...
let cmdline_opts =
  [
    ...
    ("-extra-condition-from-file", Arg.String
       (fun s -> opt_extra_condition_files :=
	  s :: !opt_extra_condition_files),
     "file Add an extra constraint for solving (from file)");		
...
let parse_symbolic_exprs_from_file fm f =
  let chan = open_in f in
  let lines = Std.input_list chan in
  close_in chan;
  List.map(fun l -> fm#parse_symbolic_expr l) lines


let parse_symbolic_expr_from_file fm f =
  let chan = open_in f in
  let buf = Std.input_all chan in
  close_in chan;
  fm#parse_symbolic_expr buf
...
let make_symbolic_init (fm:Fragment_machine.fragment_machine) 
  ...
  (fun () ->
	 ...
	 !opt_symbolic_bytes;
       (* parse expressions from the command line and file *)
       opt_symbolic_bytes_expr := (List.map(fun (addr, exp) ->
        (addr, fm#parse_symbolic_expr exp)) !opt_symbolic_bytes_expr_str) @
        (List.map (fun (addr, file) -> 
          (addr, parse_symbolic_expr_from_file fm file)) 
           !opt_symbolic_bytes_expr_files);
       List.iter (fun (addr, expr) ->
                   fm#store_symbolic_byte_expr addr expr)
        !opt_symbolic_bytes_expr;
       List.iter (fun (addr, varname) ->
