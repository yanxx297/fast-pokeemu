...
struct
  ...
  *)
  ...
  class formula_manager = object(self)
    ...
    method get_input_var s = Hashtbl.find input_vars s
	...
	| _ ->
		...
		val extra_cond_var = Hashtbl.create 0
	...
	match e with
	  ...
	  | V.Lval(V.Temp(memvar))
		...
		| V.Lval(V.Temp(v))
				when (Hashtbl.mem extra_cond_var v)
			(*Check whether this Lval is defined by a previously imported extra condition*)
		-> loop (Hashtbl.find extra_cond_var v)
	  | V.Let(var, e, exp) 
		-> (
			let var' = (
				match var with
				| V.Temp(v) -> v
				| V.Mem(_, _, _) -> failwith "Invalid Let expression") in
			Hashtbl.add extra_cond_var var' (loop e);
			let res = loop exp in
			Hashtbl.remove extra_cond_var var';
			res
			)				
      ...
      in
	match loop e with
	  | V.Constant(V.Int(_, i64)) -> (
			(*Printf.printf "eval %s to %Ld\n" (V.exp_to_string e) i64;*)
			i64)
	  | e ->
	      Printf.printf "Left with %s\n" (V.exp_to_string e);
	      failwith "Constant invariant failed in eval_expr"
	...
	if use_weak then
	  ...
	  Hashtbl.find temp_vars_unweak var
			...
			method make_symbolic_expr v ty = 
