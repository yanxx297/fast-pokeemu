...
let rec constant_fold ctx e =
  match e with
   ...
   (* Int64.div rounds towards zero. What do we want? *)
    ...
    (*  some identities *)
		| BinOp(BITOR, BinOp(BITAND, x1, Constant(Int(_, m1))), BinOp(BITAND, x2, Constant(Int(_, m2))))
			when (Int64.logor m1 m2 = 0xffffffffL
			&& x1 = x2
			&& (Vine_typecheck.infer_type None x1) = REG_32) -> 
				x1
